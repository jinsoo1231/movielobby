...
export default function SearchScreen() {
  ...
  // [제1조 2항 준수] 디바운싱(Debouncing) 로직
  // 유저가 검색창에 글자를 입력할 때마다 즉시 API를 호출하면 서버 과부하가 발생할 수 있습니다.
  // 따라서 타이핑이 끝난 후 0.5초(500ms) 동안 추가 입력이 없을 때만 `debouncedQuery`를 업데이트하여 API를 한 번만 호출하도록 최적화합니다.
  useEffect(() => {
    const handler = setTimeout(() => {
      setDebouncedQuery(query);
    }, 500); 

    // 컴포넌트가 언마운트되거나 새로운 타이핑(query)이 발생하면 기존 타이머를 취소(clear)합니다.
    return () => {
